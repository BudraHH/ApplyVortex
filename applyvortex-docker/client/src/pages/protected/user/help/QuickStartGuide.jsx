...
const StepCard = ({ number, title, description, icon: Icon }) => (
    <div className="flex flex-col h-full rounded-xl bg-white border border-slate-100 hover:border-brand-200 transition-all duration-300 group relative overflow-hidden p-6">
        ...
        <div className="relative z-10 flex items-start justify-between mb-4">
            <div className={`rounded-xl bg-slate-50 text-slate-500 group-hover:bg-brand-50 group-hover:text-brand-600 transition-colors duration-300 p-3`}>
                <Icon className="w-6 h-6" />
            </div>
            <span className="text-4xl font-black text-slate-100 group-hover:text-brand-100/50 transition-colors select-none">
                0{number}
            </span>
        </div>

        <div className="relative z-10 flex-grow space-y-4">
            <h3 className="font-bold text-slate-900 text-lg group-hover:text-brand-700 transition-colors">{title}</h3>
...
export default function QuickStartGuide() {
    ...
    return (
        ...
        >
            {/* Header */}
            <div className="flex flex-row justify-between items-center border-b border-slate-100 bg-white/50 backdrop-blur-sm sticky top-0 z-20 px-6 py-4 gap-4">

                <div>
                    <div className="flex items-center text-xs text-brand-600 font-bold uppercase tracking-wider gap-2 mb-2">
                        <Zap className="h-3.5 w-3.5" />
                        <span>Getting Started</span>
                    </div>
                    <h1 className="text-xl font-bold text-slate-900">Quick Start Guide</h1>
                </div>
                <Link to={ROUTES.HELP}>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar">
                <div className="mx-auto p-6 space-y-8 max-w-5xl">
                        ...
                        <Section title="Next Steps">
                            <ul className="space-y-3">
                                {[
                                    ...
                                    <li key={i} className="flex items-center rounded-lg bg-white border border-slate-100 hover:border-brand-200 hover:bg-brand-50/30 transition-colors group cursor-default gap-3 p-4">
                    ...
                    {/* CTA */}
                    <div className="rounded-xl relative overflow-hidden bg-slate-900 text-white flex flex-col md:flex-row items-center justify-between group p-8 gap-6">
                        ...
                        <div className="relative z-10 text-center md:text-left space-y-4">
                            <h3 className="text-2xl font-bold">Ready to automate your search?</h3>
                            <p className="text-slate-400 max-w-md">Download the agent software and let AI handle the applications while you focus on the interviews.</p>
                        </div>
