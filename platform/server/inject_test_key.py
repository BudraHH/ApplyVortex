...
async def inject_key():
    ...
    async with async_session() as session:
        ...
        # 2a. Check/Delete Existing Key with same matching Device ID (to avoid unique constraint)
        stmt = select(AgentAPIKey).where(AgentAPIKey.device_id == DEVICE_ID_FIXED)
        result = await session.execute(stmt)
        existing_device = result.scalars().all()
        for k in existing_device:
            await session.delete(k)
